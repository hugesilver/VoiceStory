...
둘은 성격이 다르므로 겹치지 않는다. 혼동하면 같은 정보를 두 번 말하게 된다.

| | 언제 소리가 나나 |
|---|---|
| `announceForAccessibility()` | **스스로 말한다.** 초점과 무관 |
| `accessibilityLabel` | **초점이 갔을 때만** 읽힌다 |

같은 정보를 화면에 표시하면서 진행 상황도 알려야 한다면, **표시는 라벨로 / 알림은 발화로** 나눈다. 둘 다 쓴다고 중복되지 않는다.

### `accessibilityLiveRegion` 을 함부로 쓰지 않는다

내용이 바뀔 때마다 자동으로 읽어주는 속성이다(주로 Android). **자주 갱신되는 값에 붙이면 끊임없이 떠든다.**

모델 다운로드 배너는 진행률이 초당 여러 번 갱신되므로 붙이지 않았다. 진행 상황은 이미 25% 단위 발화가 담당하고, 배너는 초점이 갔을 때 읽히는 라벨만 갖는다.

쓸 만한 경우는 **드물게 바뀌고 사용자가 놓치면 안 되는 값** 정도다.
